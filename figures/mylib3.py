...
Last update: 24 October, 2023 (Afternoon)
"""
...
ts = lambda duration_ms, stepsize_ms, transient_ms=0: np.arange(int(duration_ms/stepsize_ms))[int(transient_ms/duration_ms):]*stepsize_ms # ms
"""Unit: ms"""
...
def load_time_series(data_file_path: str, neuron_num: int, dtype=np.float32):
    ...
    return time_series.reshape((int(time_series.shape[0]/neuron_num), neuron_num)).T

def load_stimulus(filename):
    """Return tuple, [0]: indices of stimulated neurons, [1]: stimulus time series"""
    content = np.array(list(csv.reader(open(filename,'r',newline=''), delimiter='\t')), dtype=object)
    return np.array(content[0],dtype=int), np.array(content[1],dtype=float)
...
def power_spectrum(signal, sampling_freq, onlyPositiveFreq=True, normalized=True):
    ...
    if normalized: normalization = 1./np.sum(power_spectrum)
    else: normalization = 1
    return freqs, power_spectrum*normalization
...
class Dynamics:
    ...
    def timeseries_data_from_file(self, neuron_num, potential_filename="memp.bin",
    ...
    def membrane_potential_series(self):
        try: return load_time_series(self.__v_filename, neuron_num=self.__neuron_num)
    ...
    def recovery_variable_series(self):
        try: return load_time_series(self.__u_filename, neuron_num=self.__neuron_num)
    ...
    def presynaptic_current_series(self):
        try: return load_time_series(self.__i_filename, neuron_num=self.__neuron_num)
    ...
    def conductance_exc_series(self):
        try: return load_time_series(self.__ge_filename, neuron_num=self.__neuron_num)
    ...
    def conductance_inh_series(self):
        try: return load_time_series(self.__gi_filename, neuron_num=self.__neuron_num)
    ...
    def stochastic_current_series(self):
        try: return load_time_series(self.__stochi_filename, neuron_num=self.__neuron_num)
...
class Graphing:
    ...
    def distribution_density_plot(self, data, binsize: float, xlogscale=False, max_y_is_1=False, ax=None, **options):
        ...
        **options:
        ...
        for key, value in options.items():
            ...
            elif key == "max_val": max_val = value
    ...
    def timedep_popul_firing_rate_binned(self, spike_times, resolution_ms: float, start_t=0, end_t="auto",
                                        color="k", fr_unit="Hz", ax=None, raster_plot=None):
        ...
        else: ax.plot(x, y, c=color); ax.grid(True)
        return ax

    def timedep_popul_firing_rate_gauskern(self, spike_train, duration_ms, stepsize_ms, kernel_bandwidth_ms=1,
                                          start_t=0, end_t="auto", color="k", ax=None):
        """Firing rate unit: Hz
        Return tuple [0]: time (x), [1]: popul FR (y), [2]: matplotlib axes"""
        x = ts(duration_ms,stepsize_ms)[int(start_t/stepsize_ms):]/1000 # s
        y = get_timedep_popul_firing_rate_gauskern(spike_train,stepsize_ms,kernel_bandwidth_ms=kernel_bandwidth_ms)
        if ax == None:
            fig, ax = plt.subplots(figsize=(10,4))
            ax.set(xlabel="Time (s)", ylabel="Population\nfiring rate (Hz)")
            ax.set(xlim=(start_t/1000, end_t/1000), ylim=(0, 1.2*np.amax(y)))
            ax.text(1, 1.01, "kernel bandwidth: {} ms".format(kernel_bandwidth_ms),
                    horizontalalignment="right", verticalalignment="bottom",
                    transform=ax.transAxes, fontdict=dict(size=12))
        else: ax = ax
        ax.plot(x, y, c=color); ax.grid(True)
        return x, y, ax

    def power_spectrum(self, signal, stepsize_ms, normalized=True, freq_range="auto", ax=None, **options):
        x, y = power_spectrum(signal, 1/(stepsize_ms/1000), True, normalized)
        if freq_range=="auto": freq_range = 1/(stepsize_ms/1000)
        if ax == None:
            fig, ax = plt.subplots(figsize=(10,4))
            if normalized: ax.set(xlabel="frequency (Hz)", ylabel="spectral power density")
            else: ax.set(xlabel="frequency (Hz)", ylabel="spectral power")
            ax.set(xlim=(0,freq_range/1000), ylim=(0,1.2*np.amax(y)))
        else: ax = ax
        self.line_plot(x,y,ax=ax,**options)
        return x, y, ax
...
class SimulationData:
    ...
    def __init__(self, data_path: str, transient_time_ms=300.0, **options) -> None:
        ...
        - `rng_seed`: float
        """
        self.settings["sampling_freq_Hz"] = 1/(self.settings["dt_ms"]/1000)
        spks_fname, spks_dt_ms, spks_duration_ms = "spks.txt", self.settings["dt_ms"], self.settings["duration_ms"]
        for key, value in options.items():
            if key == "": _ = value
            elif key == "spks_fname": spks_fname = value
            elif key == "spks_dt_ms": spks_dt_ms = value
        ...
        try: self.network.adjacency_matrix_from_file(self.settings["network_file"])
        except FileNotFoundError: self.network.adjacency_matrix_from_file(os.path.join(data_path, self.settings["network_file"]))
        self.network.scale_synaptic_weights(scale=self.settings["beta"], neuron_type="all")
        ...
        self.dynamics.timeseries_data_from_file(self.settings["neuron_num"],
                os.path.join(self.data_path, "memp.bin"),
                os.path.join(self.data_path, "recv.bin"),
                os.path.join(self.data_path, "curr.bin"),
    ...
    def dataframe(self):
        ...
        data = [np.arange(1, self.settings["neuron_num"]+1),
