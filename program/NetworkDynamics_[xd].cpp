 ...
 * 
 * Compile command:
 * (MacOS) clang++ "./NetworkDynamics_[xd].cpp" -std=c++17 -O3 -o "./NetworkDynamics_[xd].o" -I"/Users/likchun/Libraries/c++/boost_1_78_0"
...
namespace model
{
    namespace Izhikevich
    {
        namespace FS
        {
            ...
            double d = 2.0*adaptation_jump_multiplying_factor;
        }
        namespace RS
        {
            ...
            constexpr const double b = 0.2;
            constexpr const double c = -65.0;
            double d = 8.0*adaptation_jump_multiplying_factor;
        }
        constexpr const double peak_potential = 30;
    }
    namespace synapse
    {
        namespace inh
        {
            ...
            constexpr const double decay_time_constant = 6.0;
        }
        namespace exc
        {
            constexpr const double threshold_potential = 0.0;
            constexpr const double decay_time_constant = 5.0;
        }
    }
}
        ...
        std::vector<double> &potential_series, std::vector<double> &recovery_series, std::vector<double> &current_synap_series)
    {
        ...
        try {
            ...
            ofs << par.init_potential << '|' << par.init_recovery << '|'
                << model::Izhikevich::FS::a << '|' << model::Izhikevich::FS::b << '|'
                << model::Izhikevich::FS::c << '|' << model::Izhikevich::FS::d << '|'
                << model::Izhikevich::RS::a << '|' << model::Izhikevich::RS::b << '|'
                << model::Izhikevich::RS::c << '|' << model::Izhikevich::RS::d << '|'
                << model::synapse::inh::threshold_potential << '|'
                << model::synapse::exc::threshold_potential << '|'
                << model::synapse::inh::decay_time_constant << '|'
                << model::synapse::exc::decay_time_constant << '|';
    ...
    int &trunc_step_inh, int &trunc_step_exc)
{
    ...
    if (par.trunc_time_inh == -1) {
        trunc_step_inh = model::synapse::inh::decay_time_constant * log(
    ...
    if (par.trunc_time_exc == -1) {
        trunc_step_exc = model::synapse::exc::decay_time_constant * log(
    ...
    std::vector<double> &spike_exp_exc, int &trunc_step_inh, int &trunc_step_exc)
{
    ...
    for (int i = 0; i < static_cast<int>(spike_exp_inh.size()); ++i) {
        spike_exp_inh[i] = exp(-i * par.dt / model::synapse::inh::decay_time_constant);
    }
    spike_exp_exc = std::vector<double>(trunc_step_exc);
    for (int i = 0; i < static_cast<int>(spike_exp_exc.size()); ++i) {
        spike_exp_exc[i] = exp(-i * par.dt / model::synapse::exc::decay_time_constant);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (now_step < total_step)
    {
        ...
        {
            if (potential[i] >= model::Izhikevich::peak_potential) {
                if (neuron_type[i] == -1) {
                    potential[i] = model::Izhikevich::FS::c;
                    recovery[i] += model::Izhikevich::FS::d;
                } else {
                    potential[i] = model::Izhikevich::RS::c;
                    recovery[i] += model::Izhikevich::RS::d;
                }
        ...
        for (int i = 0; i < network_size; ++i)
        {
            ...
            current_synap[i] = (conductance_exc[i] * (model::synapse::exc::threshold_potential - potential[i])
                + conductance_inh[i] * (model::synapse::inh::threshold_potential - potential[i]));
            ...
            if (neuron_type[i] == -1) {
                recovery[i] += model::Izhikevich::FS::a * (model::Izhikevich::FS::b * potential_prev - recovery[i]) * dt;
            } else {
                recovery[i] += model::Izhikevich::RS::a * (model::Izhikevich::RS::b * potential_prev - recovery[i]) * dt;
        }}
