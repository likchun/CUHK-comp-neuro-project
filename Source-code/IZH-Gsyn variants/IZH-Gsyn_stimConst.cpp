 ...
 * 
 * Compile command:
 * (MacOS) clang++ ./IZH_Gsyn_stimConst.cpp -std=c++17 -O3 -o ./IZH_Gsyn_stimConst.o -I"/Users/likchun/Libraries/c++/boost_1_78_0"
 * 
 */
...
int main(int argc, char **argv)
{
    ...
    while (now_step < total_step)
    {
        ...
        for (int i = 0; i < network_size; ++i)
        {
            ...
            dv1 = 0.04*potential[i]*potential[i]+5*potential[i]+140 - recovery[i] + current_synap[i] + current_const*stimulated_neuron_idx[i];// + current_stimu;
            du1 = a * (b * potential[i] - recovery[i]);

            potential_0 = potential[i] + dv1 * dt + current_stoch[i];
            recovery_0 = recovery[i] + du1 * dt;
            dv2 = 0.04*potential_0*potential_0+5*potential_0+140 - recovery_0 + current_synap[i] + current_const*stimulated_neuron_idx[i];// + current_stimu;
