 ...
 *        Purpose: for studying mutual information
 * @version 7
 * @date 2024-Mar-14
 * @note to be compiled in C++ version 11 or later with boost library 1.78.0
 * 
 * Compile command:
 * (MacOS) clang++ ./IZH_Gsyn_stimStep.cpp -std=c++17 -O3 -o ./IZH_Gsyn_stimStep.o -I"/Users/likchun/Libraries/c++/boost_1_78_0"
 * 
 */
...
std::string code_ver = "Version 7s\nLast Update: 20 April 2024\n";
...
namespace fileio
{
    ...
    void import_stimulus(Parameters &par, std::vector<int> &stimulated_neuron_idx)
    {
        std::ifstream ifs;
...
int main(int argc, char **argv)
{
    ...
    if (par.infile_stimulus != "none") {
        fileio::import_stimulus(par, stimulated_neuron_idx);
    }
    ...
    while (now_step < total_step)
    {
        ...
        for (int i = 0; i < network_size; ++i)
        {
            ...
            if (double(now_step)*dt >= 40 && double(now_step)*dt < 100) {
                stim_ON = 1;
            } else {
                stim_ON = 0;
            }
            ...
            dv1 = 0.04*potential[i]*potential[i]+5*potential[i]+140 - recovery[i] + current_synap[i] + stim_ON*current_const*stimulated_neuron_idx[i];// + current_stimu;
            du1 = a * (b * potential[i] - recovery[i]);

            potential_0 = potential[i] + dv1 * dt + current_stoch[i];
            recovery_0 = recovery[i] + du1 * dt;
            dv2 = 0.04*potential_0*potential_0+5*potential_0+140 - recovery_0 + current_synap[i] + stim_ON*current_const*stimulated_neuron_idx[i];// + current_stimu;
