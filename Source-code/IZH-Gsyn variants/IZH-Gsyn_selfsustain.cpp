 ...
 * 
 * Compile command:
 * (MacOS) clang++ ./IZH_Gsyn_selfsustain.cpp -std=c++17 -O3 -o ./IZH_Gsyn_selfsustain.o -I"/Users/likchun/Libraries/c++/boost_1_78_0"
 * 
 */
...
std::string code_ver = "Version 7b\nLast Update: 24 June 2024\n";
...
int main(int argc, char **argv)
{
    ...
    // Stop the stochastic current after t >= 500 ms
    double stoch_current_stop_ms = 500;
    int stoch_current_stop_t = int(stoch_current_stop_ms/dt);
    ...
    while (now_step < total_step)
    {
        ...
        for (int i = 0; i < network_size; ++i)
        {
            ...
            if (now_step <= stoch_current_stop_t) { current_stoch[i] = noise_intensity * norm_dist(random_generator) * sqrt_dt; } // Brownian motion
            else { current_stoch[i] = 0.0; }
