...
axes[1][0].set(ylim=(-800*.05, 800), yticks=[0, 400, 800], xlabel="$t$ (s)")
[ax.set(ylim=(-800*.05, 800), yticks=[], xlabel="$t$ (s)") for ax in axes[1][1:]]
axes[0][0].set(ylabel="50 neurons")
axes[1][0].set(ylabel="$r(t)$")

axes[0][0].text(0,1.1, "(bi)", transform=axes[0][0].transAxes)
axes[0][1].text(0,1.1, "(bii)", transform=axes[0][1].transAxes)
axes[0][2].text(0,1.1, "(biii)", transform=axes[0][2].transAxes)

fig.tight_layout()
fig.subplots_adjust(hspace=.05)
# plt.show()
fig.savefig("fig_rasterfrKappaB.pdf", dpi=400)
fig.savefig("fig_rasterfrKappaB.png", dpi=400)