...
ax.text(.26, .2, "II", fontdict=dict(font="charter", size=myFontSize0+10), ha="center", va="center", transform=ax.transData, bbox=dict(fc="w", ec="none", pad=5))
ax.text(.68, .2, "III", fontdict=dict(font="charter", size=myFontSize0+5), ha="center", va="center", transform=ax.transData, bbox=dict(fc="w", ec="none", pad=5))
...
ax_inset = ax.inset_axes([0.2, 0.6, 0.4, 0.35])
colors = ["darkorange", "g", "m"]
fmt = ["o-", "s-", "^-"]
[ax_inset.plot(gEs_gI[i], Cs_gI[i], fmt[k], c=colors[k], ms=4, lw=1, label="{}".format(gIs[i])) for k,i in enumerate([7,6,2])]
ax_inset.tick_params(axis="both", which="major", labelsize=myFontSize4)
ax_inset.set_xlabel("$g_E$", fontsize=myFontSize3)
ax_inset.set_ylabel("$C$", fontsize=myFontSize3)
ax_inset.set(xlim=xlim, xticks=np.arange(0,xlim[-1]+.1,.2), yticks=[0,.2,.4,.6,.8,1])
ax_inset.legend(title="$g_I$", fontsize=myFontSize4, title_fontsize=myFontSize3)

fig.tight_layout()
# plt.show()
fig.savefig("fig_phasediagA-2.pdf", dpi=400)
fig.savefig("fig_phasediagA-2.png", dpi=400)